...
function recognizeEmployees(names, honoredEmployee){
    // Returns greetings for both employees. 
    return names.map(function(name){
        var goodEmployee = honoredEmployee.indexOf(name) != -1;
        return goodEmployee ? `Outstanding job, ${name}!`: `Great job, ${name}!`;
    });
}